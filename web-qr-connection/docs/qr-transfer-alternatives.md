...
| --- | --- |
| 色数 | 4 色 |
| グリッド | 16x16 または 20x20 |
| 転送 | 静止フレームから開始 |
| 補正 | 四隅マーカー + 台形補正 |
| 誤り検出 | frame CRC |
| 復元 | まず FEC なし |
| 表示 | 1 フレームずつ手動または低速自動 |
| 評価 | セル認識率、フレーム認識率、転送時間 |

最初の PoC では速度よりも認識率を優先する。1 フレームを確実に読めること、距離や角度を変えてもセル単位の誤認識が測定できること、CRC でフレーム失敗を検出できることを先に確認する。その後、フレーム自動送り、色数増加、FEC 追加へ進める。
